    ...
    const std_msgs::ColorRGBA &color, const std::string &name, double scale)
{
  ...
  robot_radius_marker.header.frame_id = frame_id;
  robot_radius_marker.id = 1;
  robot_radius_marker.header.stamp = ros::Time::now();
  robot_radius_marker.type = visualization_msgs::Marker::SPHERE;
  robot_radius_marker.color.r = 0.0;
  robot_radius_marker.color.g = 0.0;
  robot_radius_marker.color.b = 1.0;
  robot_radius_marker.color.a = 0.4;
  robot_radius_marker.ns = name;
  robot_radius_marker.scale.x = scale;
  robot_radius_marker.scale.y = scale;
  robot_radius_marker.scale.z = scale;

  robot_radius_marker.pose.position.x = point.position_W.x();
  robot_radius_marker.pose.position.y = point.position_W.y();
  robot_radius_marker.pose.position.z = point.position_W.z();
  robot_radius_marker.pose.orientation.x = 0.0;
  robot_radius_marker.pose.orientation.y = 0.0;
  robot_radius_marker.pose.orientation.z = 0.0;
  robot_radius_marker.pose.orientation.w = 1.0;

  return robot_radius_marker;
}
} // namespace mav_planning
